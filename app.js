...
async function init(){
  const saved=localStorage.getItem('bb-functional-20');
  state=saved?JSON.parse(saved):await (await fetch('beers.json?v=20f',{cache:'no-store'})).json();
  render();
}
function persist(){localStorage.setItem('bb-functional-20',JSON.stringify(state));}
...
resetBtn.onclick=()=>{localStorage.removeItem('bb-functional-20');location.reload();}
...
init();
if('serviceWorker' in navigator){navigator.serviceWorker.register('./sw.js?v=20f').catch(()=>{});}
